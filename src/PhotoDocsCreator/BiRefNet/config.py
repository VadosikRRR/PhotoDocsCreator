...
class Config:
    def __init__(self) -> None:
        ...
        self.sys_home_dir = [os.path.expanduser("~"), "/workspace"][
            1
        ]  # Default, custom
        ...
        )  # wid, hei. Can be overwritten by dynamic_size in training.
        self.dynamic_size = [None, ((512 - 256, 2048 + 256), (512 - 256, 2048 + 256))][
            0
        ]  # wid, hei. It might cause errors in using compile.
        self.background_color_synthesis = (
            False  # whether to use pure bg color to replace the original backgrounds.
        )

        # Faster-Training settings
        self.mixed_precision = ["no", "fp16", "bf16", "fp8"][1]
        self.load_all = (
            False and self.dynamic_size is None
        )  # Turn it on/off by your case. It may consume a lot of CPU memory. And for multi-GPU (N), it would cost N times the CPU memory to load the data.
        ...
        self.preproc_methods = ["flip", "enhance", "rotate", "pepper", "crop"][
            : 4 if not self.background_color_synthesis else 1
        ]
